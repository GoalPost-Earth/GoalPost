...
| kind        | string   | `'reflective' \| 'ingest' \| 'import'`. Drives the mode-selector lock and the switcher's chip. `'import'` (GOAL-359) is the thread a bulk article import opens — see WF-11 step 9. |
| title       | string   | Auto-generated for ingest threads (`Ingest: <filename>`) and import threads (`Import: <n> articles into <Field>`); auto-generated by GPT-4o-mini for reflective on first exchange. Null until set. |
...
An `'import'` thread has **no** edge back to its `ArticleImportJob`. It is
reachable only through `HAS_THREAD`, and the job it follows is carried in the
seeded assistant turn's tool-call input, which the renderer reads to poll. That
is enough to make the card live but not enough to scope a *question* asked in
the thread — `get_import_status` answers about the member's recent imports
generally. Adding a `HAS_IMPORT_THREAD` edge (mirroring `HAS_INGEST_THREAD`) is
what a per-thread answer would need.

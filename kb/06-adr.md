...
## ADR-020: Roster Provenance Is a Person Node Property, Not a HAS_PERSON Edge Property (GOAL-346)

**Decision:** Extraction-found people are marked with a boolean **`Person.extractionFound`** node property, and the People roster query filters on it. The `HAS_PERSON` edge is left exactly as it was.

**Context:** `(FieldContext)-[:HAS_PERSON]->(Person)` did double duty — it was both the field's People roster (what the People section renders) and the landing spot for every person Document Ingestion (WF-10) and Bulk Article Import (WF-11) extracted. Extraction wins on volume, so the roster stopped showing the field's people and started showing the document's: on dev when this was filed, "Agentic AI Advances" read 54 of 54 extracted, and only one field anywhere still held a real user. The client's import batches are expected to add hundreds to thousands more.

**Why not the alternatives:**

- **A relationship property on `HAS_PERSON`** is the semantically correct answer — the roster is per-context, so provenance is really per-edge — and it was rejected on cost. It has **no working precedent here**: `type ConnectedTo @relationshipProperties` exists but is orphaned (`properties:` appears zero times in the SDL), and `CONNECTED_TO` edge data is surfaced through a hand-rolled `@cypher` field instead. Filtering on edge properties also means moving all three roster consumers off `people` onto `peopleConnection`, and it touches the `HAS_PERSON` schema surface that `person-pii-gate-plan-size.test.ts` pins to exactly one expansion — a gate that has already blown the 60 s `/api/graphql` ceiling once (GOAL-275, ADR-010).
- **Reusing the existing `EXTRACTED_FROM` edge as the filter**, with no new marker, is the cheapest option and it cannot express the feature. Promotion would have nowhere to write: the person still has the edge, so the roster would still hide them, and deleting the edge is out of scope (it is the Document provenance trail). The marker is nonetheless *gated* on that edge — see the reachability invariant below — so the two are coupled deliberately: the edge decides who **may** be hidden, the property decides who **is**.

**Accepted cost:** the marker is node-scoped, so promoting someone in one field un-hides them in every field they are attached to. In practice an extracted person is minted into exactly one field, and the failure direction is a person becoming *visible*, never a person being hidden.

**Consequences:**

- **The filter must name the NULL case.** The obvious spelling, `where: { NOT: { extractionFound_EQ: true } }`, compiles to `WHERE NOT (p.extractionFound = $param)`, and Cypher's three-valued logic makes that NULL — i.e. false — for every person who has no marker. Measured against dev: it matched **0 of 4** people on the North Star roster. The shipped query is `where: { OR: [{ extractionFound_EQ: false }, { extractionFound_EQ: null }] }`, which compiles to `(p.extractionFound = false OR p.extractionFound IS NULL)`. Pinned by `field-context-people-roster-filter.test.ts`.
- **`HAS_PERSON` is fail-closed and stays untouched.** It is the only Space tie a `PersonPulse` has (`PersonPrivateProfile` `contexts_SOME`), so narrowing it blanks their PII outright. The filter is a `where` on the projected nodes, nothing more.
- **`loadFieldContextRoster` deliberately does not filter.** It is the de-dup roster the extraction model matches against; filtering it would make previously-extracted people invisible to the de-duplicator and a re-extract would mint a second copy of each. Presentation roster and de-dup roster are now distinct on purpose.
- **A `:User` is never marked.** The `create_person` self-link branch attaches the *uploader* — a Space owner/member — and gives them an `EXTRACTED_FROM` edge, so without the exemption the backfill would sweep the field's one real person out of its own roster. That branch writes `false` explicitly for the same reason.
- **A person is only ever hidden when a Document still lists them.** `true` is written exclusively alongside the `EXTRACTED_FROM` edge, keyed on the same `documentId`. This is the invariant the whole design rests on: `Document.extractedPeople` is the only surface a hidden person is reachable from, so marking someone with no source Document would take them out of the roster and put them on no document — no promote, and (see below) no detach. The first cut of this change marked Bulk Article Import row authors, which has no Document in scope; two independent reviews caught that it stranded them. Both WF-11 author paths now leave the marker alone, and the row's byline stays in the roster. The people the *article* names still get marked, by the GOAL-344 ingest pass that does hold a Document.
- **`MERGE` semantics are handled in the direction that preserves a human decision.** The attach sites MERGE, so a marker written `ON CREATE SET` would be silent on re-attach. `create_person`'s enrich branch — which an extraction re-run and a member's assistant request both land in — clears the marker only when there is no `documentId`: a member re-adding someone is curating the roster, a re-extract must not undo promotes.
- **Detach got harder, and that is a real regression, not a neutral consequence.** `RemovePersonFromFieldContext` is wired to exactly one surface, the field page's People roster, which now filters these people out. Before this change a member could hand-remove the 54 extracted people from a field; now they can promote one from the document drawer but cannot detach from there. Since `HAS_PERSON` still grants `privateProfile` reach (below), losing the remove path matters more than losing the view. Adding detach beside promote in the extracted-people list is the follow-up; it needs its own confirm step and activity `Log`.
- **Promotion reuses `addPersonToFieldContext` unchanged** — no new server surface. The person is already on `HAS_PERSON`, so the MERGE is a no-op and the meaningful write is clearing the marker; the resolver's existing Space edit gate (`canEditContent`) and activity `Log` cover it.
- **Existing data needs a backfill** — `npm run backfill:extraction-found`, seeded from `EXTRACTED_FROM`, skipping `:User` and anyone not on a roster, idempotent and reversible with `--undo`. Extraction-found people carrying no `EXTRACTED_FROM` edge are deliberately left unmarked: no signal separates them from hand-added people, and staying visible is the recoverable failure.
- **PII reach is unchanged and that is a live question, not an oversight.** `HAS_PERSON` still unlocks a person's gated `privateProfile` to anyone who can view the context (`kb/02-user-roles.md` branch 5). Filtering the roster does not narrow that reach — at import volume a field now grants PII reach to hundreds of extracted people who are invisible in its UI. Whether that reach *should* narrow is a separate decision, deliberately not taken here.

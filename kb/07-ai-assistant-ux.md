...
**A write tool may produce something that still needs a second human step, and must SAY so.** `propose_promise_weave` (GOAL-342) is the case to copy. Approving its card does not establish a promise weave — it writes one with `status: 'proposed'`, which the member then confirms or dismisses in the field's "Promise weaves" section (`kb/04-state-machines.md`). Three things follow, and all three are part of the tool, not polish on top of it:

- The **approval copy** says what approving actually does (`describeWriteAction` returns "Propose … — it arrives as a proposal for you to confirm or dismiss"), and `entityKindLabel` names the thing a "promise weave proposal", not a "promise weave". A card that reads "Weave X" misdescribes the action the member is agreeing to.
- The **tool result** carries `awaitingReview: true` and a message naming where the second gate lives, so the model cannot narrate a confirmed connection nobody agreed to. `search_promise_weave` returns the same flag on `proposed` rows for the same reason.
- The **anchor comes from session state, and the card's copy comes from the graph.** The tool injects `ctx.fieldContextId` over anything the model emits, and resolves the pulse titles and person name itself rather than letting the model supply them — a card that names two pulses while the write touches ten is a gate in name only. Be clear about what that injection is worth, though: `fieldContextId` arrives on the request body and the approval round-trip replays client-supplied args, so the injection scopes the MODEL, and the server-side `canEditContext` is the actual boundary. Never relax the gate on the strength of the injection.

...
export default function WeSpaceFieldsPage() {
  ...
  // sibling Space pages from each other's listeners.
  useEffect(() => {
    if (!weSpaceId) return
    return onOpenAddFieldContextModal((detail) => {
      if (detail.spaceId !== weSpaceId) return
  ...
  // Graph view (default)
  return (
    <div className="flex flex-col h-full w-full overflow-hidden bg-gp-surface dark:bg-gp-surface-dark transition-colors">
      {queryError && (

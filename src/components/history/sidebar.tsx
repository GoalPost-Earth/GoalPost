...
export function Sidebar({
  ...
  return (
    <nav
      className={`fixed md:static left-0 top-0 w-64 h-screen md:h-auto z-30 flex flex-col border-r border-glass-border bg-glass-bg/50 backdrop-blur-md pt-6 pb-4 dark:bg-glass-bg/40 dark:border-white/10 transform transition-transform duration-300 md:translate-x-0 ${
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto px-2 space-y-1 scroller pt-2">
        <div className="flex items-center justify-between px-3 py-2 mb-2">
          <span className="text-xs font-bold text-slate-400 uppercase tracking-widest dark:text-white/40">
            Views
          </span>
          <button
            onClick={onClose}
            className="md:hidden p-1.5 rounded-md hover:bg-white/30 dark:hover:bg-white/20 transition-colors flex-shrink-0"
      ...
      {/* Status Footer */}
      <div className="px-4 pt-4 border-t border-glass-border mt-auto dark:border-white/10 flex-shrink-0">
        <div className="flex items-center gap-3 p-2 rounded-lg bg-white/40 border border-white/50 shadow-sm backdrop-blur-sm dark:bg-white/5 dark:border-white/5">
          <div className="size-2 rounded-full bg-accent-glow animate-pulse flex-shrink-0"></div>

...
- "documents" / "source documents" / "uploaded files/articles" / "what documents are associated with these resources" / "what documents led to these pulses" / "where did this resource come from" → document RESOURCES: \`(:ResourcePulse)\` filtered \`WHERE doc.sourceFilename IS NOT NULL\`. A document is a KIND OF RESOURCE (GOAL-354), NOT its own label — there is no \`:Document\` — and it is NOT identified by \`resourceType\`: an imported article attaches its file to the row's own Resource (GOAL-356), so it is typed 'article'/'book' and \`resourceType = 'document'\` would silently drop it. Reach them TWO ways and include BOTH: a field's uploaded docs via \`(ctx:FieldContext)-[hp:HAS_PULSE]->(doc:ResourcePulse) WHERE doc.sourceFilename IS NOT NULL\`, and the source doc(s) behind specific pulses via \`(p:FieldPulse)-[ef:EXTRACTED_FROM]->(doc:ResourcePulse)\`. For "documents associated with the Resources in <field>", anchor the field, OPTIONAL MATCH its \`ResourcePulse\`s via HAS_PULSE, then OPTIONAL MATCH each resource's \`EXTRACTED_FROM\` document — bind and RETURN every edge so both the resources and their source documents render. Caption a document by its \`sourceFilename\` or \`title\`; never surface its id/sourceBlobKey/sourceBlobUrl.

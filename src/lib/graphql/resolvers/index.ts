...
const resolvers = {
  ...
  // GraphQLResolveInfo
  //
  // The `*RelationshipResolvers` spreads below are fallbacks for parents
  // materialized by a custom root resolver; they defer to @neo4j/graphql's
  // own projection whenever the library resolved the parent. See
  // ./relationship-fallbacks.ts and ./projection.ts.
  ...
  MeSpace: spaceRelationshipResolvers,

  WeSpace: spaceRelationshipResolvers,
  ...
  Space: {
    __resolveType: async (obj: Record<string, unknown>) => {
      ...
      // Library-generated interface paths (Person.ownsSpaces,
      // SpaceMembership.space, the `spaces` root, …) stamp the concrete type
      // name as `__resolveType` on every row — @neo4j/graphql's own generated
      // resolver just reads it, and this custom resolver shadows that. Reading
      // it here keeps those paths zero-cost instead of one label lookup per
      // space row. Same pattern as FieldPulse.__resolveType below (GOAL-286).
      if (obj.__resolveType === 'MeSpace' || obj.__resolveType === 'WeSpace') {

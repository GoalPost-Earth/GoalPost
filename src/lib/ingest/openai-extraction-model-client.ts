...
function buildSystemPrompt(input: ExtractionModelInput): string {
  ...
  return [
    ...
    '- Also set authorLabel to how the document credits that person, in its own terms and in as few words as possible: "Author", "Interviewee", "Keynote speaker", "Co-author". This becomes the visible label on the authorship link, so write it as a reader would say it — never a database term, never a sentence. Use null when the document credits them without saying how.',
    '- For every pulse, set relatedPeople to the people the document names as related to it but who are NOT its author (subjects, contributors, beneficiaries), and relatedOrganizations to the organizations related to it (e.g. the cooperative offering a resource). Each entry is an object: name must match one you emitted (or EXISTING PEOPLE / EXISTING ORGANIZATIONS), and label says how the document relates them to the pulse in its own terms and in as few words as possible — "Interviewed", "Cited", "Beneficiary", "Offers this resource", "Publisher". This is how named people and orgs stay connected to the resources and stories they belong to, and the label is what a reader sees on the link. Use null for a label the document does not state, null for the whole list when none apply, and never repeat authorName in relatedPeople.',

...
describe('Langchain Agent', () => {
  ...
  describe('Vector Retrieval', () => {
    it('should perform RAG using the neo4j vector retriever', async () => {
      ...
      const input = 'Recommend me a person who is passionate about freedom'
      ...
      const sessionRes = await graph.query(
        ...
        RETURN r.input AS input, r.output AS output, r.source AS source,
          ...
          [ (r)-[:CONTEXT]->(m) | m.firstName + ' ' + m.lastName ] AS people
      `,
        { sessionId }
      )
      ...
      if (sessionRes) {
        ...
        for (const person of sessionRes[0].people) {
          if (output.toLowerCase().includes(person.toLowerCase())) {
            found = true
          }
        }

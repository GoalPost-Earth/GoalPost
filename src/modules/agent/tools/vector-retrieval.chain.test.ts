...
describe('Vector Retrieval Chain', () => {
  ...
  it('should provide a recommendation', async () => {
    ...
    const rephrasedQuestion = 'Recommend a person who is an introvert'
    ...
    console.log(
      '🚀 ~ file: vector-retrieval.chain.test.ts:103 ~ output:',
      output
    )
    ...
    const res = await graph.query(
      `
        ...
        RETURN s.id AS session, r.input AS input, r.output AS output,
          ...
          [ (r)-[:CONTEXT]->(m) | m.firstName + ' ' + m.lastName ] AS context
    ...
    for (const name of first.context) {
